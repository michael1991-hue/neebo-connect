...
charge.observeLevel(60); charge.observeLevel(80)
check(!charge.isCharging, "a battery rise is not treated as charging")
charge.observeLevel(40)
check(!charge.isCharging, "a battery fall is not treated as charging")
charge.observePowerState(charging: true)
check(charge.isCharging, "only the band charging report counts")
charge.observePowerState(charging: false)
check(!charge.isCharging, "charging ends when the band says so")
...
let skinFrame = BluetoothPolicy.customFrame(Data([0,0,0,0x5F,0,0x63,0,0x3F,1]))
check(skinFrame.skinCelsius.map { abs($0 - 31.9) < 0.05 } == true, "last two bytes are skin temperature in tenths")
...
check(FamilyRelation.wire("Dad") == "dad" && FamilyRelation.wire("dad") == "dad", "Dad is sent in the form family sharing already accepts")
check(FamilyRelation.wire("Grandad") == "Grandad", "a typed name is kept")
...
check(HistoryChartPolicy.closerZoom(than: 6 * 3600) == 3600 && HistoryChartPolicy.widerZoom(than: 3600) == 6 * 3600, "zoom steps 6 hours to 1 hour")
check(HistoryChartPolicy.widerZoom(than: 6 * 3600) == 0, "wider than 6 hours is the selected date")
...
let liveX = HistoryChartPolicy.xScale(from: ahead, to: now)
...
let skinned = SavedMeasurement.mapped(time: now, heartRate: 90, oxygen: 97, source: "family-share", skinCelsius: 30.1)
...
check(SharedAlertLog.event(previous: "none", next: "sensor", wasAcknowledged: false, acknowledged: false) == nil, "sensor warnings are not copied into family alert history")
check(SharedAlertLog.shares(SavedEvent(time: now, kind: "critical", title: "They need your attention", detail: "High heart-rate alert", heartRate: 180)), "a heart-rate alert is shared")
check(!SharedAlertLog.shares(SavedEvent(time: now, kind: "connection", title: "Connection lost", detail: "Reconnecting", heartRate: nil)), "connection events stay on the monitoring phone")
