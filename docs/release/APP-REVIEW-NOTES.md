...
Review up to 30 calendar days of recorded readings and events, select points on the history graph, add notes, and export a CSV when you choose. Add a profile photo and use a parent-marked sleep or awake timer to record your observations.
...
- Build: 0.9.4 (13). Login: none. In-app purchases: none.
...
- Home shows readings only when usable data arrives. History separates readings and events. Sleep is manually marked and is not derived from a sensor.
...
Capture Home; graph selection/history; event filters and recovery event; Bluetooth device selection; simplified alert settings; profile/sleep timer. Use the screenshot dimensions requested by App Store Connect for the selected supported devices. Do not show unimplemented remote sharing or imply the animated lungs measure breathing.
